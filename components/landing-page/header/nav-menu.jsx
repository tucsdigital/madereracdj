            ...
            item.child ? (
              <NavigationMenu.Item key={`item-${index}`} value={item}>
                ...
                <NavigationMenu.Content
                  className={cn(
                    "w-full  rounded-md border bg-popover text-popover-foreground shadow-lg border-solid border-border"
                  )}
                >
                  {item.child && (
...
const ListItem = React.forwardRef(
  ({ className, children, title, childItem, ...props }, forwardedRef) => (
    <NavigationMenu.Link asChild>
      <Link
        className={cn(
          " select-none   text-base  font-medium text-default-600 rounded-md flex  items-center gap-2 mb-4 last:mb-0  leading-none no-underline outline-hidden transition-colors  hover:text-primary  focus:text-primary",
          className
        )}
        {...props}
        ref={forwardedRef}
