...
export async function POST(request) {
  try {
    ...
    const res = await createVentaEngine({ actor, ventaData: venta, origen });
    return NextResponse.json({ ok: true, ventaId: res.id, numeroPedido: res.numeroPedido, flags: res.flags }, { status: 200 });
  } catch (err) {
