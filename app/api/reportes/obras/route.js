...
const row = (item, kind) => ({ numero: item?.numero || item?.numeroPedido || String(item?.id || "").slice(-8), fecha: kind === "obra" ? getObraReferenceDate(item) : item?.fechaCreacion || item?.fecha || "", cliente: String(item?.cliente?.nombre || item?.clienteNombre || "Sin cliente").toUpperCase(), estado: kind === "obra" ? String(item?.estado || "pendiente").replaceAll("_", " ") : "Pagada", total: kind === "obra" ? obraTotal(item) : Number(item?.total) || 0 });
const makeRows = (items, kind) => items.map((item) => row(item, kind)).sort((a, b) => dateOf(a.fecha).getTime() - dateOf(b.fecha).getTime());
const total = (rows) => rows.reduce((sum, item) => sum + item.total, 0);
const table = (rows, empty) => rows.length ? `${rows.map((item, index) => `<tr><td>${index + 1}</td><td>${escape(item.numero)}</td><td>${escape(dateLabel(item.fecha))}</td><td class="client">${escape(item.cliente)}</td><td>${escape(item.estado)}</td><td class="amount">${escape(money(item.total))}</td></tr>`).join("")}<tr class="total"><td colspan="5">Total</td><td class="amount">${escape(money(total(rows)))}</td></tr>` : `<tr><td colspan="6" class="empty">${escape(empty)}</td></tr>`;
const section = (title, rows, empty) => `<section><h2>${escape(title)}</h2><table><thead><tr><th>#</th><th>Comprobante</th><th>Fecha</th><th>Cliente</th><th>Estado</th><th>Importe</th></tr></thead><tbody>${table(rows, empty)}</tbody></table></section>`;
const pdf = ({ key, selected, obras, ventas }) => { const cards = []; if (selected.includes("obras")) cards.push(["Obras completadas o pagadas", money(total(obras.confirmed))], ["Comisión obras 2,5%", money(total(obras.confirmed) * RATE / 100)]); if (selected.includes("ventas")) cards.push(["Ventas terminadas de pagar", money(total(ventas.paid))], ["Comisión ventas 2,5%", money(total(ventas.paid) * RATE / 100)]); return `<!doctype html><html lang="es"><head><meta charset="utf-8"><style>@page{size:A4;margin:12mm}*{box-sizing:border-box}body{font:11px Arial;color:#172033}.head{padding:17px 18px;border-radius:16px;background:#12233c;color:#fff}.over{font-size:9px;letter-spacing:1px;color:#bcd2ef;text-transform:uppercase}h1{margin:4px 0 0;font-size:20px}.cards{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:14px 0}.card{border:1px solid #dce5ef;border-radius:12px;padding:12px}.label{font-size:9px;color:#64748b;text-transform:uppercase;font-weight:bold}.value{font-size:20px;font-weight:bold;margin-top:7px}section{margin-top:19px;page-break-inside:avoid}h2{font-size:14px}table{width:100%;border-collapse:separate;border-spacing:0;border:1px solid #dde5ee;border-radius:10px;overflow:hidden}th{padding:9px 8px;background:#eaf0f7;text-align:left;font-size:9px;text-transform:uppercase}td{padding:9px 8px;border-top:1px solid #edf1f5}.amount{text-align:right;font-weight:bold}.client{text-transform:uppercase}.total td{background:#eaf0f7!important;font-weight:bold;text-transform:uppercase}.empty{text-align:center;color:#64748b}</style></head><body><header class="head"><div class="over">Maderas Caballero</div><h1>Reporte mensual - ${escape(monthLabel(key))}</h1></header><div class="cards">${cards.map(([label, value]) => `<div class="card"><div class="label">${escape(label)}</div><div class="value">${escape(value)}</div></div>`).join("")}</div>${selected.includes("obras") ? `${section("Obras confirmadas", obras.confirmed, "No hay obras completadas o pagadas en este mes.")}${section("Obras pendientes", obras.pending, "No hay obras pendientes en este mes.")}` : ""}${selected.includes("ventas") ? section("Ventas pagadas", ventas.paid, "No hay ventas pagadas en este mes.") : ""}</body></html>`; };

export async function POST(request) {
  try {
    const decoded = await verifyFirebaseToken(String(request.headers.get("authorization") || ""));
    if (decoded?.email !== ADMIN_EMAIL) return NextResponse.json({ ok: false, error: "No tenés permisos para enviar reportes." }, { status: 403 });
    ...
    const obras = { confirmed: makeRows(obrasAll.filter((obra) => isCompletedOrPaid(obra) && inRange(lastPayment(obra) || obra?.fechaCompletada || obra?.fechaFinalizacion || obra?.fechaModificacion, start, end)), "obra"), pending: makeRows(obrasAll.filter((obra) => inRange(getObraReferenceDate(obra), start, end) && !isCompletedOrPaid(obra)), "obra") };
    ...
    const ventas = { paid: makeRows(ventasAll.filter((venta) => deriveVentaEstadoPago({ total: venta?.total, abonado: calcVentaAbonado(venta) }) === "pagado" && inRange(venta?.fechaPagoConfirmado || lastPayment(venta), start, end)), "venta") };
    const key = month.key; const commission = (total(obras.confirmed) + total(ventas.paid)) * RATE / 100; const apiKey = String(process.env.RESEND_API_KEY || "").trim(); const from = String(process.env.RESEND_FROM || "").trim();
