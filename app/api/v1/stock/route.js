...
export async function GET(request) {
  try {
    ...
    for (const pid of productoIds) {
      ...
      const componentIds = getComboComponentIds(data);
      if (componentIds.length > 0) {
        const productoById = new Map();
        for (const componentId of componentIds) {
          const componentSnap = await getDoc(doc(db, "productos", componentId));
          if (componentSnap.exists()) {
            productoById.set(componentId, { id: componentSnap.id, ...(componentSnap.data() || {}) });
          }
        }
        data = withDerivedComboStock(data, productoById);
      }
