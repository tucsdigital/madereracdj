...
import { collection, query, where, limit, getDocs, addDoc, doc, setDoc } from "firebase/firestore";
...
export async function POST(request) {
  try {
    ...
    if (!email || !email.includes("@")) {
      return withCors(NextResponse.json({ error: "email_requerido" }, { status: 400 }));
    }
    if (!nombre) {
      return withCors(NextResponse.json({ error: "nombre_requerido" }, { status: 400 }));
    }
    ...
    const payloadCreate = {
      ...
      creadoEn: nowIso,
      actualizadoEn: nowIso,
      // Campos de dirección quedan para endpoints de direcciones
    };
