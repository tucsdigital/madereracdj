                        ...
                        <>
                          <div>
                            ...
                            <GoogleAddressInput
                              ...
                              onChange={({ address, locality, lat, lng, mapsUrl }) => setPresupuestoEdit((prev) => ({ ...prev, direccionEnvio: address, localidadEnvio: locality || prev.localidadEnvio, direccionMapsUrl: mapsUrl, direccionLat: lat, direccionLng: lng }))}
...
function FormularioConvertirVenta({ presupuesto, onCancel, onSubmit }) {
  ...
  const schema = yup.object().shape({
    ...
    estadoPagoConversion: yup
      .string()
      .oneOf(["pagado", "parcial", "pendiente"])
      .required("Selecciona el estado de pago"),
    formaPago: yup.string().when("estadoPagoConversion", {
      is: (val) => val === "pagado" || val === "parcial",
      then: (s) => s.required("Selecciona la forma de pago"),
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4 bg-card rounded-lg p-4 border border-gray-200 shadow-sm">
          ...
          {tipoEnvioSeleccionado && (
            <>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  {tipoEnvioSeleccionado === "retiro_local" ? "Fecha de retiro" : "Fecha de entrega *"}
                </label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  {tipoEnvioSeleccionado === "retiro_local" ? "Horario de retiro (opcional)" : "Rango horario *"}
                </label>
                <Input
        ...
        <div className="space-y-2 bg-card rounded-lg p-4 border border-gray-200 shadow-sm">
          ...
          <div>
            ...
            <select
              {...register("estadoPagoConversion")}
              className={`w-full border rounded-md px-3 py-2 ${
                errors.estadoPagoConversion ? "border-red-500" : "border-gray-300"
              }`}
            >
            ...
            {errors.estadoPagoConversion && (
              <span className="text-red-500 text-xs">
                {errors.estadoPagoConversion.message}
              </span>
            )}
          </div>
          {watch("estadoPagoConversion") !== "pendiente" && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Forma de pago
              </label>
          ...
          {watch("estadoPagoConversion") === "parcial" && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Monto abonado
              </label>
