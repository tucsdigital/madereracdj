...
export function calcularPreciosProducto(producto, {
  ...
  const planedUnitPrice = redondear ? roundToHundreds(planedUnitRaw) : planedUnitRaw;
  const planedTotalPrice = redondear ? roundToHundreds(planedTotalRaw) : planedTotalRaw;

  const finalUnitPrice = cepillado && aplicaCepillado ? planedUnitPrice : baseUnitPrice;
  const finalTotalPrice = cepillado && aplicaCepillado ? planedTotalPrice : baseTotalPrice;

  return {
    categoria,
    unidad,
    cantidad: cantidadNum,
    appliedPlaning: Boolean(cepillado && aplicaCepillado),
    baseUnitPrice,
    planedUnitPrice,
    finalUnitPrice,
    baseTotalPrice,
