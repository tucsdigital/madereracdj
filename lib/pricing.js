...
export function computeLineBase(product = {}) {
  ...
  const machimbreDeckPrecioTotal =
    isMachimbreOrDeck(product) && (unidadIndicaTotal || unidad === "");

  // Líneas donde el precio ya viene totalizado (incluye cantidad)
  if (precioIncluyeCantidad || machimbreDeckPrecioTotal || (categoria === "Maderas" && unidadIndicaTotal)) {
    return price;
  }
