export const DOCUMENTACION_TEXTOS = {
  ...
  envio: {
    asuntoEmail: (numero, titulo) => {
      ...
      const base = num ? `Documento para confirmación - ${num}` : "Documento para confirmación";
    ...
    cuerpoEmailHtml: ({ clienteNombre, numero, titulo, link, logoUrl, descripcionHtml = "" }) => {
      ...
      const num = String(numero || "").trim();
      const t = String(titulo || "").trim();
      const asunto = num ? `Documento para confirmación - ${num}${t ? ` - ${t}` : ""}` : `Documento para confirmación${t ? ` - ${t}` : ""}`;
