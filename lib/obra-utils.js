...
export const calcularPrecioMachimbre = ({ alto, largo, cantidad, precioPorPie }) => {
...
export const calcularPrecioCorteMadera = ({ alto, ancho, largo, precioPorPie }) => {
  const altoNum = Number(alto) || 0;
  const anchoNum = Number(ancho) || 0;
  const largoNum = Number(largo) || 0;
  const precioPorPieNum = Number(precioPorPie) || 0;
  
  if (altoNum <= 0 || anchoNum <= 0 || largoNum <= 0 || precioPorPieNum <= 0) return 0;
  
  const volumen = altoNum * anchoNum * largoNum;
  return Math.round(volumen * precioPorPieNum);
};

export const calcularPrecioProductoObra = ({ unidadMedida, alto, largo, valorVenta, cantidad }) => {
...
export const generarContenidoImpresion = (obra, presupuesto, modoCosto, movimientos = []) => {
  ...
  if (movimientos && Array.isArray(movimientos)) {
    totalMovimientos = movimientos.reduce((acc, m) => acc + Number(m.monto || 0), 0);
  }
                ...
                ${obra.materialesCatalogo.map((p) => {
                  ...
                  return `
                    <tr>
                      ...
                      <td class="numeric-value">${esMadera ? largoNum : "-"}</td>
                      <td class="numeric-value">${medidaValor != null ? medidaValor.toLocaleString("es-AR") : "-"}</td>
