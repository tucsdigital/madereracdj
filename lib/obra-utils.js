...
// Helpers de visualización para unidades de medida de productos de obra.
// Los productos de obra se guardan con campos heterogéneos: unidadMedida,
// unidad, largo/largoNum, alto, m2, ml, cantidad. Estas funciones normalizan
// esos datos para mostrar Alto/Largo/ML/M2 reales en UI y en el remito.
export const textoDimension = (valor) => {
  const n = Number(valor);
  if (!Number.isFinite(n) || n <= 0) return "-";
  return Number.isInteger(n) ? String(n) : String(Math.round(n * 100) / 100);
};

export const detalleMedidaProducto = (p) => {
  const unidadRaw = String(p?.unidadMedida || p?.unidad || "UN").trim();
  const unidad = unidadRaw.toUpperCase();
  const alto = Number(p?.alto);
  const largo = Number(p?.largo ?? p?.largoNum);
  const ancho = Number(p?.ancho);
  const m2 = Number(p?.m2);
  const ml = Number(p?.ml);
  const cantidad = Number(p?.cantidad) || 1;
  if (unidad === "M2") {
    const altoTxt = textoDimension(Number.isFinite(alto) && alto > 0 ? alto : ancho);
    const largoTxt = textoDimension(largo);
    const m2calc = Number.isFinite(m2) && m2 > 0 ? m2 : (Number.isFinite(alto) && alto > 0 && Number.isFinite(largo) && largo > 0 ? alto * largo * cantidad : (Number.isFinite(ancho) && ancho > 0 && Number.isFinite(largo) && largo > 0 ? ancho * largo * cantidad : 0));
    const m2txt = m2calc > 0 ? ` (${Number.isInteger(m2calc) ? m2calc : Math.round(m2calc * 100) / 100} m²)` : "";
    return {
      unidad,
      altoTxt,
      largoTxt,
      medidaTxt: m2txt ? m2txt.replace(/^ \(|\)$/g, "") : "-",
      sub: `Alto ${altoTxt} · Largo ${largoTxt}${m2txt}`,
    };
  }
  if (unidad === "ML") {
    const largoTxt = textoDimension(largo);
    const mlcalc = Number.isFinite(ml) && ml > 0 ? ml : (Number.isFinite(largo) && largo > 0 ? largo * cantidad : 0);
    const mltxt = mlcalc > 0 ? ` (${Number.isInteger(mlcalc) ? mlcalc : Math.round(mlcalc * 100) / 100} ml)` : "";
    return {
      unidad,
      altoTxt: "-",
      largoTxt: largoTxt,
      medidaTxt: mltxt ? mltxt.replace(/^ \(|\)$/g, "") : largoTxt,
      sub: `Largo ${largoTxt}${mltxt}`,
    };
  }
  return { unidad: unidadRaw || "UN", altoTxt: "-", largoTxt: "-", medidaTxt: "-", sub: null };
};
          ...
          const filas = bloque.productos
            .map((p) => {
              ...
              const filaBase = `
                  <tr>
                    <td>
                      <div><strong>${p.nombre || "-"}</strong></div>${medida.sub ? `<div class="product-details">${medida.sub}</div>` : ""}
                    </td>
                    <td style="text-align: center;">${cantNum}</td>
                    <td style="text-align: center;">${medida.unidad}</td>
                    <td style="text-align: center;">${medida.altoTxt}</td>
                    <td style="text-align: center;">${medida.largoTxt}</td>
      ...
      ? `
        ...
        .map((p) => {
          ...
          const filaBase = `
                    <tr>
                      <td>
                        <div class="product-name">${p.nombre || "-"}</div>${medida.sub ? `<div class="product-details">${medida.sub}</div>` : ""}
                      </td>
                      <td class="numeric-value">${cantNum}</td>
                      <td class="numeric-value">${medida.unidad}</td>
                      <td class="numeric-value">${medida.altoTxt}</td>
                      <td class="numeric-value">${medida.largoTxt}</td>
