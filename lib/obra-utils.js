...
export const generarContenidoImpresion = (
  ...
  const hayBloques = Array.isArray(obra?.bloques) && obra.bloques.some((b) => Array.isArray(b?.productos) && b.productos.length > 0);
  ...
  const bloquesSeccionHTML = hayBloques
    ? (() => {
        ...
        const totalesGenerales = obra.bloques.reduce(
          (acc, bloque) => {
            if (!Array.isArray(bloque?.productos) || bloque.productos.length === 0) return acc;
            const subtotal = bloque.productos.reduce((a, p) => a + Number(p.precio || 0), 0);
            const descuentoTotal = bloque.productos.reduce(
              (a, p) => a + Number(p.precio || 0) * (Number(p.descuento || 0) / 100),
              0
            );
        ...
        const bloquesHTML = obra.bloques
          ...
          .map((bloque, idx) => {
                ...
                return `
                  <tr>
                    ...
                    <td style="text-align: center;">${
                      medidaValor != null ? medidaValor.toLocaleString("es-AR") : "-"
                    }</td>
            ...
            return `
              <div class="section">
                <div class="section-title">${bloque.nombre || `Bloque ${idx + 1}`}</div>
                ...
                <div class="totals-section">
                  <div class="totals-content">
                    <div class="description-field">
                      <div class="info-label" style="margin-bottom: 4px;">Descripción del Bloque</div>
                      <textarea readonly class="description-placeholder">${bloque.descripcion || ""}</textarea>
                    </div>
                    <div class="totals-right">
                      <div class="total-row">
                        <span class="total-label">Subtotal:</span>
                        <span class="total-value">${formatearNumeroArgentino(subtotal)}</span>
                      </div>
                      <div class="total-row">
                        <span class="total-label">Descuento total:</span>
                        <span class="total-value">${formatearNumeroArgentino(descuentoTotal)}</span>
                      </div>
                      <div class="total-row">
                        <span class="total-label">Total:</span>
                        <span class="total-value grand-total">${formatearNumeroArgentino(total)}</span>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        return `
          <div class="section">
            <div class="section-title">Presupuesto por Bloques</div>
          </div>
          ${bloquesHTML}
        `;
      })()
