...
export const formatDate = (date) => {
  ...
  if (typeof date === "string" && /^\d{4}-\d{2}-\d{2}$/.test(date)) {
    ...
    d = new Date(Date.UTC(y, (m || 1) - 1, day || 1, 12, 0, 0));
  } else {
    d = new Date(date);
  }
...
export const buildLocalizedPath = (pathname, lang = "en") => {
  const knownLocales = ["bn", "en", "ar"];
  if (knownLocales.some((l) => pathname === `/${l}` || pathname.startsWith(`/${l}/`))) {
    return pathname;
  }
...
export const useLocalizedPath = () => {
  ...
  const lang = knownLocales.includes(maybeLang) ? maybeLang : "en";

  return (path) => buildLocalizedPath(path, lang);
};
