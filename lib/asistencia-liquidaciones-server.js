...
function resolveEmployeeStartDate(empleado) {
  const candidates = [
...
function isDateBeforeEmployeeStart(dateInput, empleado) {
...
function calcularPremioAsistenciaMensualServer({ empleado, asistencias, monthKey }) {
  ...
  if (!year || !monthInput) {
    return {
      ...
      premio: 0,
      motivos: [],
      estadoLabel: "Sin reconocimiento",
      fechaCorte: null,
      esMesActual: false,
    };
  }
  ...
  if (descuentoTardanzas > 0) {
    motivos.push(
      `Llegadas tarde ${tardanzas} · ajuste $${descuentoTardanzas.toLocaleString("es-AR")}`,
    );
  }
...
function normalizePremioAsistenciaSnapshot(input) {
  return {
    ...
    premioBase: Number(input?.premioBase || 0),
    descuentoTardanzas: Number(input?.descuentoTardanzas || 0),
    ...
    tardanzas: Number(input?.tardanzas || 0),
    minutosTarde: Number(input?.minutosTarde || 0),
    diasEsperados: Number(input?.diasEsperados || 0),
    motivos: Array.isArray(input?.motivos) ? input.motivos : [],
    estadoLabel: String(input?.estadoLabel || "Sin reconocimiento"),
  };
}
