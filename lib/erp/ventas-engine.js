...
export async function updateVentaEngine({ actor, ventaId, ventaData, origen = "sistema_ventas" }) {
  ...
  const result = await db.runTransaction(async (t) => {
    ...
    const prodRefs = ids.map((pid) => db.collection("productos").doc(pid));
    ...
    for (let i = 0; i < ids.length; i += 1) {
      ...
      if (!prodSnap?.exists) throw new Error(`Producto ${pid} no encontrado`);
      const prod = prodSnap.data() || {};

      const prevQty = prevGrouped.get(pid) || 0;
      const nextQty = nextGrouped.get(pid) || 0;
