...
async function revertirAplicacionSaldoAFavorEnCuentas({
  transaction,
  db,
  movimientoId,
  cuentasAplicadas,
  ...
  for (const c of Array.isArray(cuentasAplicadas) ? cuentasAplicadas : []) {
    const cuentaId = String(c?.cuentaId || "").trim();
    const montoAplicado = toNumber(c?.montoAplicado);
    if (!cuentaId || montoAplicado <= 0) continue;
    porCuenta.set(cuentaId, toNumber(porCuenta.get(cuentaId)) + montoAplicado);
  }

  const cuentaIds = Array.from(porCuenta.keys());
  for (const cuentaId of cuentaIds) {
    ...
    const montoAplicado = Number(toNumber(porCuenta.get(cuentaId)).toFixed(2));
    if (montoAplicado <= 0) continue;

    const yaRevertido = pagosArr.some(
      (p) =>
        p?.saldoAFavorReversion === true &&
        String(p?.saldoAFavorMovimientoId || "") === movimientoId &&
        Number(toNumber(p?.monto).toFixed(2)) === Number((-montoAplicado).toFixed(2))
    );
    if (yaRevertido) continue;

    const existePagoAplicado = pagosArr.some(
      (p) =>
        p?.saldoAFavorAplicado === true &&
        p?.pagoGlobalProveedor === true &&
        p?.saldoAFavorReversion !== true &&
        Number(toNumber(p?.monto).toFixed(2)) === montoAplicado &&
        toDateKey(p?.fecha) === fechaBase
    );
    if (!existePagoAplicado) continue;
    ...
    const pagoReversion = stripUndefined({
      ...
      saldoAFavorReversion: true,
      saldoAFavorMovimientoId: movimientoId,
    });

    pagosArr.push(pagoReversion);

    const montoTotal = toNumber(cuenta?.monto);
    const montoPagadoNuevo = pagosArr.reduce((sum, pago) => sum + toNumber(pago?.monto), 0);
...
export async function registrarPagoGlobalProveedorEngine({
  ...
  const res = await db.runTransaction(async (t) => {
    ...
    const selectedRaw = Array.isArray(cuentasSeleccionadas) ? cuentasSeleccionadas : [];
    ...
    if (selectedIds.length > 0) {
      if (selectedIds.length > 450) {
        const err = new Error("Demasiadas cuentas seleccionadas para aplicar en una sola operación (limite 450).");
        err.status = 400;
        throw err;
      }
      const snaps = [];
      for (const id of selectedIds) {
        ...
        snaps.push({ id, ref, data: snap.data() || {} });
      }
      cuentas = snaps
        .filter((c) => String(c.data?.tipo || "") === "proveedor")
        .filter((c) => String(c.data?.proveedorId || "") === provId)
...
export async function quitarSaldoAFavorProveedorEngine({
  ...
  const isUsd = Boolean(pagoEnDolares);
  const usdRateRaw = valorOficialDolar ?? null;
  const usdRate = isUsd && Number.isFinite(Number(usdRateRaw)) ? Number(usdRateRaw) : null;
  const comprobantesArr = Array.isArray(comprobantes) ? comprobantes : [];
  ...
  const res = await db.runTransaction(async (t) => {
    ...
    t.set(
      ...
      stripUndefined({
        ...
        fechaRegistro: new Date().toISOString(),
        pagoEnDolares: isUsd,
        valorOficialDolar: isUsd ? usdRate : null,
...
export async function agregarSaldoAFavorProveedorEngine({
  actor,
  proveedorId,
  monto,
  fecha,
  ...
  const montoAgregar = toNumber(monto);
  if (montoAgregar <= 0) throw new Error("monto inválido");

  const fechaMov = toDateKey(fecha) || new Date().toISOString().split("T")[0];
  const metodoMov = String(metodo || "Efectivo");
  const notasMov = String(notas || "");
  const isUsd = Boolean(pagoEnDolares);
  const usdRateRaw = valorOficialDolar ?? null;
  const usdRate = isUsd && Number.isFinite(Number(usdRateRaw)) ? Number(usdRateRaw) : null;
  ...
  const res = await db.runTransaction(async (t) => {
    ...
    const proveedor = proveedorSnap.data() || {};
    const saldoAntes = toNumber(proveedor.saldoAFavor);
    const saldoDespues = Number((saldoAntes + montoAgregar).toFixed(2));
    const deltaSaldo = Number((saldoDespues - saldoAntes).toFixed(2)); // positivo
    if (deltaSaldo === 0) {
      return {
        ok: true,
        proveedorId: provId,
    ...
    t.set(
      ...
      stripUndefined({
        ...
        saldoAFavorAntes: Number(saldoAntes.toFixed(2)),
        saldoAFavorDespues: saldoDespues,
        montoAgregado: Number(deltaSaldo.toFixed(2)),
      })
    );
  ...
  const res = await db.runTransaction(async (t) => {
    ...
    if (tipo === "pagoGlobal") {
      ...
      for (const cuentaId of cuentaIds) {
        ...
        cuentaSnaps.push({ cuentaId, cuentaRef, cuentaSnap });
      }

      for (const item of cuentaSnaps) {
        if (!item.cuentaSnap.exists) continue;
        const cuenta = item.cuentaSnap.data() || {};
        const montoTotal = toNumber(cuenta?.monto);
        const montoPagadoActual = toNumber(cuenta?.montoPagado);
        const montoAplicado = toNumber(porCuenta.get(item.cuentaId));
        if (montoAplicado <= 0) continue;
      ...
      if (String(pago?.direccion || "").trim() === "aplicacion_a_cuentas") {
        await revertirAplicacionSaldoAFavorEnCuentas({
          transaction: t,
          db,
          movimientoId: id,
          cuentasAplicadas: pago?.cuentasAplicadas,
          actor,
          fechaBase,
          notasAnulacion,
        });
...
export async function registrarPagoCuentaProveedorEngine({
  ...
  const res = await db.runTransaction(async (t) => {
    ...
    let movRef = null;
    if (excedente > 0) {
      proveedorRef = db.collection("proveedores").doc(provId);
      proveedorSnap = await t.get(proveedorRef);
      if (!proveedorSnap.exists) {
        const err = new Error("Proveedor no encontrado");
        err.status = 404;
        throw err;
      }
      proveedor = proveedorSnap.data() || {};
      saldoAntes = toNumber(proveedor.saldoAFavor);
      saldoDespues = Number((saldoAntes + excedente).toFixed(2));
      deltaSaldo = Number((saldoDespues - saldoAntes).toFixed(2));
      movRef = db.collection("pagosProveedores").doc();
    }
    const nuevoPago = stripUndefined({
...
export async function mutarPagoCuentaProveedorEngine({
  ...
  const res = await db.runTransaction(async (t) => {
    ...
    const pagoAntes = stripUndefined({
      monto: toNumber(target?.monto),
      fecha: toDateKey(target?.fecha) || null,
      metodo: String(target?.metodo || ""),
      notas: String(target?.notas || ""),
      responsable: String(target?.responsable || ""),
      fechaRegistro: String(target?.fechaRegistro || ""),
      pagoEnDolares: Boolean(target?.pagoEnDolares),
      valorOficialDolar: target?.valorOficialDolar ?? null,
      comprobantes: Array.isArray(target?.comprobantes) ? target.comprobantes : [],
      pagoGlobalProveedor: Boolean(target?.pagoGlobalProveedor),
      pagoGlobalId: String(target?.pagoGlobalId || ""),
    });

    let pagoDespues = null;
    if (act === "delete") {
      ...
      const updated = pagosArr[index] || {};
      pagoDespues = stripUndefined({
        monto: toNumber(updated?.monto),
        fecha: toDateKey(updated?.fecha) || null,
        metodo: String(updated?.metodo || ""),
        notas: String(updated?.notas || ""),
        responsable: String(updated?.responsable || ""),
        fechaRegistro: String(updated?.fechaRegistro || ""),
        pagoEnDolares: Boolean(updated?.pagoEnDolares),
        valorOficialDolar: updated?.valorOficialDolar ?? null,
        comprobantes: Array.isArray(updated?.comprobantes) ? updated.comprobantes : [],
        pagoGlobalProveedor: Boolean(updated?.pagoGlobalProveedor),
        pagoGlobalId: String(updated?.pagoGlobalId || ""),
      });
    }
    ...
    t.set(
      ...
      stripUndefined({
        ...
        pagoAntes,
        pagoDespues,
        cuentaAnulada: Boolean(cuenta?.anulada === true || String(cuenta?.estadoOperativo || "") === "anulada"),
        montoPagadoAntes: Number(toNumber(cuenta?.montoPagado).toFixed(2)),
        montoPagadoDespues: Number(montoPagadoNuevo.toFixed(2)),
      })
    );
