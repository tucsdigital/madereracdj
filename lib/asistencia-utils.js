...
function resolveScheduledStartTime(empleado) {
  const candidates = [
    empleado?.horaIngreso,
    empleado?.horaEntrada,
    empleado?.horarioIngreso,
    empleado?.horarioEntrada,
    empleado?.inicioJornada,
  ];
  for (const candidate of candidates) {
...
function resolveLateToleranceMinutes(empleado) {
  const candidates = [
    empleado?.toleranciaLlegadaMinutos,
    empleado?.toleranciaTardanzaMinutos,
    empleado?.premioAsistenciaToleranciaMinutos,
    empleado?.minutosToleranciaIngreso,
  ];
...
function getLateArrivalBreakdown(dayData, empleado) {
  const horaIngresoEsperada = resolveScheduledStartTime(empleado);
  const toleranciaMinutos = resolveLateToleranceMinutes(empleado);
  const horaLlegada = normalizeTimeString(
  ...
  const legacyLlegadaTarde = Boolean(
    dayData?.llegoTarde ?? dayData?.llegadaTarde ?? dayData?.tarde ?? false,
  );
  const legacyMinutosTarde = Math.max(
    Number(dayData?.minutosTarde ?? dayData?.demoraMinutos ?? 0),
    0,
  );

  const minutosEsperados = timeStringToMinutes(horaIngresoEsperada);
  const minutosLlegada = timeStringToMinutes(horaLlegada);
  const minutosDemora =
    minutosEsperados != null && minutosLlegada != null
      ? Math.max(minutosLlegada - minutosEsperados, 0)
      : legacyMinutosTarde;
  const minutosExcesoTolerancia =
    minutosEsperados != null && minutosLlegada != null
      ? Math.max(minutosDemora - toleranciaMinutos, 0)
      : legacyLlegadaTarde
        ? Math.max(legacyMinutosTarde - toleranciaMinutos, legacyMinutosTarde > 0 ? 0 : 1)
        : 0;
  const llegoTarde =
    minutosEsperados != null && minutosLlegada != null
      ? minutosExcesoTolerancia > 0
      : legacyLlegadaTarde;

  return {
    llegoTarde,
    minutosTarde: roundMoney(minutosExcesoTolerancia),
    minutosDemora: roundMoney(minutosDemora),
    notaTarde,
    horaIngresoEsperada,
    horaLlegada,
    toleranciaMinutos: roundMoney(toleranciaMinutos),
  };
}
...
function normalizeTardanzaDetalle(item) {
  return {
    id: String(item?.id || ""),
...
function resolveTardanzasDetalle(resumenMes, comprobante) {
  const source = Array.isArray(resumenMes?.tardanzasDetalle)
    ? resumenMes.tardanzasDetalle
    : Array.isArray(comprobante?.tardanzasDetalle)
      ? comprobante.tardanzasDetalle
      : [];

  return source
    .map(normalizeTardanzaDetalle)
    .sort((a, b) => {
      const compare = String(b.dateIso || "").localeCompare(String(a.dateIso || ""));
      if (compare !== 0) return compare;
      return String(a.id || "").localeCompare(String(b.id || ""));
    });
}

function buildTardanzasDetalleSectionHtml(tardanzasDetalle) {
  return `
      <div class="section">
        <div class="section-head">
          <div>
            <div class="section-kicker">Llegadas tarde</div>
            <div class="section-title">Registros del periodo</div>
          </div>
          <div class="section-note">${tardanzasDetalle.length} registro${tardanzasDetalle.length === 1 ? "" : "s"} detectado${tardanzasDetalle.length === 1 ? "" : "s"}</div>
        </div>
          ...
          <table class="late-table">
            <thead>
              <tr>
                ...
                <th class="right">Minutos tarde</th>
                <th>Justificacion</th>
              </tr>
            </thead>
            <tbody>
              ${tardanzasDetalle.length > 0 ? tardanzasDetalle.map((item) => `
              <tr>
                <td>${escapeHtml(formatDateAR(item.dateIso))}</td>
                <td>${escapeHtml(item.dayLabel || "-")}</td>
                <td>${escapeHtml(item.horaLlegada || "-")}</td>
                <td>${escapeHtml(item.horaEsperada || "-")}</td>
                <td class="right">${Number(item.minutosTarde || 0).toLocaleString("es-AR")} min</td>
                <td>${escapeHtml(item.justificacion || "-")}</td>
              </tr>`).join("") : `
              <tr>
                <td colspan="6" class="empty-cell">No hay llegadas tarde registradas en este mes.</td>
              </tr>`}
            </tbody>
          </table>
        </div>
...
export function getDayPaymentBreakdown({ dayData, empleado, dateInput }) {
  ...
  const lateBreakdown = getLateArrivalBreakdown(dayData, empleado);
  ...
  return {
    ...
    minutosDemora: lateBreakdown.minutosDemora,
    notaTarde: lateBreakdown.notaTarde,
    horaIngresoEsperada: lateBreakdown.horaIngresoEsperada,
    horaLlegada: lateBreakdown.horaLlegada,
    toleranciaMinutos: lateBreakdown.toleranciaMinutos,
  };
}
...
export function buildLiquidacionAsistenciaHtml({ empleado, resumenMes, comprobante }) {
  ...
  const motivos = Array.isArray(resumenMes?.premioAsistencia?.motivos)
    ...
    : Array.isArray(comprobante?.premioAsistencia?.motivos)
      ...
      : [];
  const tardanzasDetalle = resolveTardanzasDetalle(resumenMes, comprobante);
...
<html lang="es">
  <head>
    ...
    <style>
      * {
      ...
      .late-table td:nth-child(1) {
        width: 15%;
      }
      .late-table th:nth-child(2),
      .late-table td:nth-child(2) {
        width: 13%;
      }
      .late-table th:nth-child(3),
      .late-table td:nth-child(3) {
        width: 14%;
      }
      .late-table th:nth-child(4),
      .late-table td:nth-child(4) {
        width: 16%;
      }
      .late-table th:nth-child(5),
      .late-table td:nth-child(5) {
        width: 14%;
      }
      .late-table th:nth-child(6),
      .late-table td:nth-child(6) {
        width: 28%;
      }
      .attendance-table th,
      .attendance-table td {
  ...
  <body>
    <div class="sheet">
      ...
      ${buildTardanzasDetalleSectionHtml(tardanzasDetalle)}
...
export function buildControlAsistenciaValeHtml({ empleado, resumenMes, comprobante }) {
  const nombre = escapeHtml(empleado?.nombre || "Empleado");
  const sector = escapeHtml(empleado?.sector || "-");
  const monthLabel = escapeHtml(resumenMes?.labelMes || comprobante?.labelMes || "-");
  const emittedAt = comprobante?.generatedAt
  ...
  const cumplimiento = Number(
    resumenMes?.premioAsistencia?.porcentaje ||
      comprobante?.premioAsistencia?.porcentaje ||
      0,
  );
  const presentes = Number(
    resumenMes?.premioAsistencia?.presentes ||
      comprobante?.premioAsistencia?.presentes ||
      0,
  );
  const medias = Number(
    resumenMes?.premioAsistencia?.medias || comprobante?.premioAsistencia?.medias || 0,
  );
  const ausentes = Number(
    resumenMes?.premioAsistencia?.ausentes ||
      comprobante?.premioAsistencia?.ausentes ||
      0,
  );
  const justificadas = Number(
    resumenMes?.premioAsistencia?.justificadas ||
      comprobante?.premioAsistencia?.justificadas ||
      0,
  );
  const tardanzas = Number(
    resumenMes?.premioAsistencia?.tardanzas ||
      comprobante?.premioAsistencia?.tardanzas ||
      0,
  );
  const estado = escapeHtml(
    resumenMes?.premioAsistencia?.estadoLabel ||
      comprobante?.premioAsistencia?.estadoLabel ||
      "Sin reconocimiento",
  );
  const premio = formatCurrencyAR(
    resumenMes?.premioAsistencia?.premio || comprobante?.premioAsistencia?.premio || 0,
  );
  const tardanzasDetalle = resolveTardanzasDetalle(resumenMes, comprobante);
  const cumplimientoLabel = `${Number(cumplimiento || 0).toLocaleString("es-AR", {
...
<html lang="es">
  <head>
    <meta charset="UTF-8" />
    <title>CONTROL Asistencia del periodo</title>
    <style>
      * {
        box-sizing: border-box;
      }
  ...
  <body>
    <div class="sheet">
      <div class="header">
        <div>
          <div class="eyebrow">Control interno</div>
          <div class="title">CONTROL Asistencia del periodo</div>
          <div class="subtitle">${nombre} · ${monthLabel} · Sector ${sector}</div>
        </div>
        <div class="meta">
          <div><strong>Emitido:</strong> ${escapeHtml(emittedAt)}</div>
        </div>
      ...
      <div class="table-shell">
        <table>
          ...
          <tbody>
            <tr>
              <td>${escapeHtml(cumplimientoLabel)}</td>
              <td>${presentes.toLocaleString("es-AR")}</td>
              <td>${medias.toLocaleString("es-AR")}</td>
              <td>${ausentes.toLocaleString("es-AR")}</td>
              <td>${justificadas.toLocaleString("es-AR")}</td>
              <td>${tardanzas.toLocaleString("es-AR")}</td>
              <td class="estado">${estado}</td>
              <td class="premio">${escapeHtml(premio)}</td>
            </tr>
          </tbody>
        </table>
      </div>
...
export function getPremioAsistenciaConfig(empleado) {
  return {
    ...
    horaIngreso: resolveScheduledStartTime(empleado),
    toleranciaMinutos: resolveLateToleranceMinutes(empleado),
  };
}
