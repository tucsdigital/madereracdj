...
export async function createVenta({ db, user, formData, origen = "sistema_ventas" }) {
  ...
  const result = await runTransaction(db, async (t) => {
    ...
    const productoRefs = grouped.map((entry) => doc(db, "productos", entry.productoId));
    const productoSnaps = await Promise.all(productoRefs.map((r) => t.get(r)));
    const productosData = grouped.map((entry, idx) => {
      const snap = productoSnaps[idx];
      if (!snap.exists()) {
        throw new Error(`Producto no encontrado: ${entry.nombre || entry.productoId}`);
      }
      return { entry, productoRef: productoRefs[idx], data: snap.data() || {} };
    });

    for (const { entry, productoRef, data } of productosData) {
...
export async function updateVenta({ db, user, ventaId, ventaData, origen = "sistema_ventas" }) {
  ...
  const result = await runTransaction(db, async (t) => {
    ...
    const idsArr = Array.from(allIds.values());
    const productoRefs = idsArr.map((productoId) => doc(db, "productos", productoId));
    const productoSnaps = await Promise.all(productoRefs.map((r) => t.get(r)));
    const productosData = idsArr.map((productoId, idx) => {
      const snap = productoSnaps[idx];
      if (!snap.exists()) throw new Error(`Producto ${productoId} no encontrado`);
      return { productoId, productoRef: productoRefs[idx], prod: snap.data() || {} };
    });

    for (const { productoId, productoRef, prod } of productosData) {
